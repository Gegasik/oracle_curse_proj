drop procedure CREATECATEGORY;
drop procedure DeleteCategoryById;
drop procedure DeleteCategoryByTitle;
drop procedure UpdateCategory;
--****************************************************************************************************
drop procedure CreateCinemaHall;
drop procedure DeleteCinemaHallById;
drop procedure DeleteCinemaHallByTitle;
drop procedure UpdateCinemaHall;
******************************************************************************************************
drop procedure CreateFilm;
drop procedure DeleteFilmById;
drop procedure DeleteFilmByName;
drop procedure UpdateFilm;
******************************************************************************************************
drop procedure addCategoryToFilm;
drop procedure deleteCategoryFromFilm;
******************************************************************************************************
drop procedure CreatePlace;
drop procedure DeletePlaceById;
drop procedure UpdatePlace;
******************************************************************************************************
drop procedure CreateSession;
drop procedure DeleteSessionById;
drop procedure UpdateSession;
******************************************************************************************************
drop procedure BUYTICKET
drop procedure DELETETICKET